...
def run_pipeline_over_whole_datasets(hit_dfs, stats, xy_hyperparams, rz_hyperparams):
    ...
    def _run_pipelines(dfs, _type):
        ...
        for df in tqdm(dfs, desc=desc):
            ...
            xy_combo_rz_est_tracks_to_hits = Hough2d_combined_pipeline(xy_est_tracks_to_hits, df, rz_hyperparams,
                                                                       second_transform_is_rz=True)
            rz_combo_xy_est_tracks_to_hits = Hough2d_combined_pipeline(rz_est_tracks_to_hits, df, xy_hyperparams,
                                                                       second_transform_is_rz=False)
            df['track'] = df['xy_track'];
